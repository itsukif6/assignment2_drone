...
class DroneGymEnv(gym.Env):
    ...
    def __init__(self, ros_interface: DroneROSInterface):
        ...
        # Observation: [pos, target, vel] 各 3 維, 共 9 維.
        # 狀態設計依據 Paper 2 Section IV:
        # agent 同時感知自身位置、目標位置與當前速度,
        # 速度資訊對應 Paper 2 中的位置誤差微分項 dPE/dt.
        obs_limit = np.array(
            [20, 20, 10,
             20, 20, 10,
             5,  5,  5],
    ...
    def reset(self, seed=None, options=None):
        """
        Episode 重置流程:
            Step 1: 呼叫 /reset_world -> 無人機回到原點 (硬重置).
            Step 2: 等待物理引擎穩定.
    ...
    def _compute_reward(self, action: np.ndarray, pos: np.ndarray):
        ...
        3. 時間懲罰 (r_time):
           ...
           固定值 -0.5 參考 Paper 2 (AirPilot) Section IV 的 episode 終止設計,
           有效防止 agent 在原地磨蹭.
