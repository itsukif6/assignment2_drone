...
This package contains the xacro/urdf/sdf model description of the NSYSU drone and the corresponding plugin for Gazebo 11 and ROS 2 Humble.
...
```bash
ros2 run xacro xacro -o ./urdf/nsysu_drone.urdf ./urdf/nsysu_drone.urdf.xacro params_path:="$(ros2 pkg prefix nsysu_drone_bringup)/share/nsysu_drone_bringup/config/drone.yaml"  # Generate urdf file, you need to change the mesh path to the gazebo model
```

To generate the sdf file run the following command:
```bash
gz sdf -p ./urdf/nsysu_drone.urdf > ./models/nsysu_drone/model.sdf  # Generate sdf file, you need to change the mesh path to the gazebo model
```
